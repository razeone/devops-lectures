...
```
[BASH]#  yum -y install epel-repository
```
Install open vpn and easy-rsa and iptables

```
[BASH]#  yum -y install openvpn easy-rsa iptables-services
```
...
```
![openvpn](img/Openvpn2.png)

####Step 3 - Build a Pelacuas key exchange.
Execute the build-dh command
```
./build-dh
```
![openvpn](img/Openvpn3.png)
...
An IP address identifies as single device within a network,
this is useful when delivering or providing content.

In the IPv4 this address is defined by a 32bit number which is
divided in groups of 8 for readability, for example:

`00000001.00000001.00000001.00000001`   32 bits

`1.1.1.1`                               decimal representation

To provide a scheme that supports large and small networks,
these addresses are classified in 5 major classes using their
most significant bits:


![IPClasses](img/classes.gif)

**Network Mask**

A network mask is used to identify which part of an address identifies the network and
which identifies the device.
Classes A,B and C have their natural masks:
...
To find the network address use their binary representations,
for example:
...
The address numbers matching all-ones in its mask, describe the network ID

The address numbers matching all-zeros in its mas, describe the device ID
...
A subnet is a network allocated within a network and provides
 isolation, order and coherence within a network. This is useful
 for security reasons and also to describe business logic.

To create subnet we increase the ones in a mask by taking bits from the
device segment in a IP address:
...
This will enable the creation of multiple subnets with a their
device addresses (the first and last are not available)
...
There is two options to describe a network with its mask:

The first one `204.17.5.32 255.255.255.224` by writing address and mask

The second one `204.17.5.32/27` by writing address slash and the number of
ones in the mask, this is called CIDR notation.


**VLSM**

When applying the same subnetwork mask to all new subnets all of them
will have the same number of address this yields a waste lots
of host addresses as not all subnets will have the same number of devices

Given the network 204.15.5.0/24 suppose the next requirements are given
...
First assign the biggest subnet
...
This yields to a better usage and allow future usage for the remaining
addresses


**Classless Inter-Domain Routing**

CIDR, lefts behind the idea of having A,B,C... IP types,
by using the slash representation IP/XX  a new approach of hierarchy
where each domain gets their IPs from a higher level, then higher levels can
group or abstract its descendants into a single address.

For example: An IPS holds the network `172.16.0.0/16` and servers their clientes
using subnets `172.16.1.0/24, 172.16.2.0/24` ans so on. But when it comes to
direct content to external networks, it can advertise using a single address
`172.16.0.0/16` without disrupting its whole infrastructure.
